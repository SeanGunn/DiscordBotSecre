...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES")) {
        return message.reply("Missing Permissions!").then(message  => { message.delete({ timeout: 10000 }) }).catch(console.error);

    }

    if(isNaN(args[0])){
        return message.reply("Enter a number after the command to clear messages.").then(message  => { message.delete({ timeout: 10000 }) }).catch(console.error);
